...
**Bead**: gt-0g1
**Date**: 2026-03-08
**Scope**: Frontier agent memory and identity systems — MemGPT/Letta, Voyager, Stanford Generative Agents, O-Mem, CoALA, Reflexion, and reflection patterns. Key questions: How do agents build persistent world models? What lies beyond context windows? How does identity persist across sessions?
...
Recent surveys (ACM TOIS 2025, ICLR 2026 MemAgents Workshop) converge on a taxonomy drawn from cognitive science:
...
| **Episodic** | Autobiographical | Records of past events and experiences | Stanford Generative Agents, Reflexion |
| **Semantic** | Factual knowledge | Stable knowledge about the world and self | CoALA, O-Mem, knowledge graphs |
| **Procedural** | Skills / how-to | Executable behaviors and action patterns | Voyager skill library, code-based agents |
...
MemGPT established the dominant metaphor: LLM context windows are RAM; external stores are disk. The agent manages its own virtual memory through function calls that page data between tiers. This metaphor is now near-universal:
...
### 3.1 MemGPT / Letta — The Memory Operating System
...
**Letta V1 (2025-2026)**: Moved away from heartbeat-driven loops toward native reasoning with direct assistant message generation. The architecture now follows modern ReAct/Claude Code patterns rather than the original MemGPT loop.

### 3.2 Voyager — The Skill Library
...
- **Action Space**: Internal (reasoning, retrieval, learning) + External (grounding in environments)
- **Decision Cycle**: Plan (propose → evaluate → select) → Execute → Observe → repeat

**Key insight on procedural memory**: Writing to procedural memory (modifying the agent's own code or behavior patterns) is "significantly riskier" than other memory types. This matches Gas Town's experience — formula modifications have higher blast radius than data changes.
...
### 3.5 Reflexion — Verbal Reinforcement Learning
...
### 3.6 O-Mem — Omni Memory for Personalization
...
**The missing layer**: No current system builds genuine causal world models — representations that predict "if I do X, Y will happen" beyond pattern matching. Current agents accumulate correlations, not causal structure. This is an open research frontier.
...
Recent benchmarks (March 2026) show fact-based memory achieves **91% reduction in p95 response latency** vs. full-context processing, with comparable or better accuracy. The verdict: external memory with smart retrieval beats longer context windows for persistent agents.
...
5. **Sleep-time memory consolidation**: Between sessions, a background process could consolidate raw bead notes into structured knowledge. This maps to Letta's sleep-time compute.
...
5. **Compression vs. fidelity**: The Gas City effect algebra already models this tradeoff. Memory consolidation IS lossy compression. The question is whether the fidelity preorder from Gas City's formal model can be applied to memory operations.
...
- Park et al. (2023). "Generative Agents: Interactive Simulacra of Human Behavior." UIST '23. arXiv:2304.03442
- Sumers et al. (2024). "Cognitive Architectures for Language Agents." TMLR. arXiv:2309.02427
- Shinn et al. (2023). "Reflexion: Language Agents with Verbal Reinforcement Learning." NeurIPS 2023. arXiv:2303.11366
- Li et al. (2025). "O-Mem: Omni Memory System for Personalized, Long Horizon, Self-Evolving Agents." arXiv:2511.13593

### Surveys and Frameworks
- Zhang et al. (2024). "A Survey on the Memory Mechanism of Large Language Model-based Agents." ACM TOIS. arXiv:2404.13501
- "Memory in LLM-based Multi-agent Systems: Mechanisms, Challenges, and Collective." TechRxiv 2025.
- "Rethinking Memory Mechanisms of Foundation Agents in the Second Half: A Survey." arXiv:2602.06052 (2026).
...
### Recent Advances
- "Beyond the Context Window: Cost-Performance Analysis of Fact-Based Memory vs. Long-Context LLMs." arXiv:2603.04814 (2026).
...
- Letta V1 Architecture Blog (2025): Rearchitecting from MemGPT to modern agent patterns.
- Qu et al. (2024). "RISE: Recursive Introspection for Self-improvement." NeurIPS 2024.

### Gas Town Internal
- Gas City Design Synthesis (2026-03-08)
- Gas City Formula Engine Vision (2026-03-08)
